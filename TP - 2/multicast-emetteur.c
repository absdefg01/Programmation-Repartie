...
#define HELLO_GROUP "225.112.14.15"
...
int main(int argc, char** argv){
	...
	char multicastIPString[128];           // adresse de groupe
...
if (argc != 3){ perror("Mauvaise utilisation de la commande: <adresse de groupe> <numero de port>\n"); exit(1);}
	...
	strncpy(multicastIPString, argv[1], MAXHOSTLEN);
...
ecode = getaddrinfo(multicastIPString,serverPort,&hints,&res);
	if (ecode) {
		fprintf(stderr,"getaddrinfo: %s\n", gai_strerror(ecode));
		exit(1);
	}
/* Le paramètre hints contient des précisions pour renseigner la prise socket */
